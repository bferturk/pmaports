...
def check_environment_variables():
    """ Make sure that all environment variables from GitLab CI are present,
        and exit right here when we are not running in CI for a merge request,
        or if a variable is missing. """
    # Do nothing when not in a merge request
    if "CI_MERGE_REQUEST_ID" not in os.environ:
        print("*** MR settings check skipped (CI_MERGE_REQUEST_ID is not set)")
        exit(0)

    # Make sure we have all variables set
    keys = ["CI_MERGE_REQUEST_PROJECT_PATH",
